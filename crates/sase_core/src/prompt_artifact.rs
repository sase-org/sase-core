        ...
        let candidate = resolved
            ...
            .flat_map(|(record_index, target)| {
                ...
                [record.raw_ref.as_str(), record.expanded_ref.as_str()]
                    ...
                    .map(move |token| (*record_index, target.as_str(), token))
            })
            .filter(|(_, _, token)| {
                prompt[cursor..next_protected].starts_with(token)
            })
            .max_by_key(|(_, _, token)| token.len());
        if let Some((record_index, target, token)) = candidate {
            rewritten.push('[');
            rewritten.push_str(token);
            rewritten.push_str("](");
            rewritten.push_str(target);
            rewritten.push(')');
            linked[record_index] = true;
            cursor += token.len();
            continue;
        }
