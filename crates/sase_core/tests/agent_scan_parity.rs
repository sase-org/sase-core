...
fn build_ace_run_done(root: &Path) {
    ...
    write_json(
        ...
        &json!({
            ...
            "video_paths": ["/tmp/videos/demo.mp4"],
...
#[test]
fn running_record_carries_custom_role_display_labels() {
    let tmp = tempdir().unwrap();
    let root = build_fixture_tree(&tmp.path().join("projects"));
    write_json(
        &root
    ...
    let rec = record_by_timestamp(&snapshot, TS_ACE_RUN_RUNNING);
    let meta = rec.agent_meta.as_ref().unwrap();
    let custom_role = meta.agent_family_custom_role.as_ref().unwrap();

    assert_eq!(
        custom_role.get("id").and_then(Value::as_str),
        Some("tester")
    );
    assert_eq!(
        custom_role.get("label").and_then(Value::as_str),
        Some("TESTING")
    );
    assert_eq!(
        custom_role.get("done_label").and_then(Value::as_str),
        Some("TESTED")
    );
    ...
    assert_eq!(
        indexed_meta.agent_family_custom_role,
        meta.agent_family_custom_role
    );
}
...
fn done_record_parses_done_marker() {
    ...
    assert_eq!(done.video_paths, vec!["/tmp/videos/demo.mp4"]);
