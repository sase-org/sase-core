...
- *(provider-priority)* expose pool eligibility and reservation policy bindings
...
- *(bead-action)* expose close/keep policy and assigned-bead finalizer bindings
...
- *(gate-followup)* expose settled gate coder-handoff recovery decisions
...
- *(notifications)* export plus-one append and upsert bindings
...
- *(provider-usage)* export observation validation and public snapshot projection
...
- bind `validate_finalizer_plan` and `authenticate_finalizer_plan` for sealed-plan integrity
