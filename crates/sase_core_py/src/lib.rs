...
use sase_core::artifact::{
    ...
    upsert_artifact_link as core_upsert_artifact_link,
...
/// Add or update an artifact node, link, or payload.
#[pyfunction]
#[pyo3(name = "artifact_add")]
fn py_artifact_add<'py>(
    py: Python<'py>,
    index_path: &str,
    request: &Bound<'py, PyDict>,
) -> PyResult<PyObject> {
    let add_request = artifact_add_request_from_pydict(request)?;
    let index = PathBuf::from(index_path);
    let result = py
        .allow_threads(|| {
            let mut store = core_open_artifact_store(&index)?;
            match add_request {
                ArtifactAddRequest::Node(request) => {
                    core_upsert_artifact_node(&mut store, request)
                }
                ArtifactAddRequest::Link(request) => {
                    core_upsert_artifact_link(&mut store, request)
                }
                ArtifactAddRequest::Payload(payload) => {
                    core_upsert_artifact_payload(&mut store, payload)
                }
            }
        })
...
/// Remove or tombstone an artifact node or link.
#[pyfunction]
#[pyo3(name = "artifact_remove")]
fn py_artifact_remove<'py>(
    py: Python<'py>,
    index_path: &str,
    request: &Bound<'py, PyDict>,
) -> PyResult<PyObject> {
    let remove_request = artifact_remove_request_from_pydict(request)?;
    let index = PathBuf::from(index_path);
    let result = py
        .allow_threads(|| {
            let mut store = core_open_artifact_store(&index)?;
            match remove_request {
                ArtifactRemoveRequest::Node(request) => {
                    core_remove_artifact_node(&mut store, request)
                }
                ArtifactRemoveRequest::Link(request) => {
                    core_remove_artifact_link(&mut store, request)
                }
            }
        })
        .map_err(artifact_store_error_to_pyerr)?;
...
/// List artifacts with optional `ArtifactQueryWire` filters.
#[pyfunction]
#[pyo3(name = "artifact_list", signature = (index_path, query = None))]
fn py_artifact_list<'py>(
    py: Python<'py>,
    index_path: &str,
    query: Option<&Bound<'py, PyDict>>,
) -> PyResult<PyObject> {
...
/// Show one artifact detail record.
#[pyfunction]
#[pyo3(name = "artifact_show")]
fn py_artifact_show<'py>(
    py: Python<'py>,
    index_path: &str,
    artifact_id: &str,
) -> PyResult<PyObject> {
...
#[pyfunction]
#[pyo3(name = "artifact_graph", signature = (index_path, options = None))]
fn py_artifact_graph<'py>(
    py: Python<'py>,
    index_path: &str,
    options: Option<&Bound<'py, PyDict>>,
) -> PyResult<PyObject> {
    ...
    let graph = py
        .allow_threads(|| {
            let store = core_open_artifact_store(&index)?;
            core_artifact_materialize_graph(&store, options)
        })
        .map_err(artifact_store_error_to_pyerr)?;
...
#[pyo3(
    ...
    signature = (index_path, artifact_path, request = None)
)]
fn py_artifact_upsert_path<'py>(
    py: Python<'py>,
    index_path: &str,
    artifact_path: &str,
    request: Option<&Bound<'py, PyDict>>,
...
#[pyfunction]
#[pyo3(name = "artifact_doctor", signature = (index_path, options = None))]
fn py_artifact_doctor<'py>(
    py: Python<'py>,
    index_path: &str,
    options: Option<&Bound<'py, PyDict>>,
) -> PyResult<PyObject> {
    let options = match options {
        Some(dict) => artifact_wire_from_pydict::<ArtifactDoctorOptionsWire>(
            dict,
            "options",
            "ArtifactDoctorOptionsWire",
        )?,
        None => ArtifactDoctorOptionsWire::default(),
    };
    let index = PathBuf::from(index_path);
    let doctor = py
        .allow_threads(|| {
            let store = core_open_artifact_store(&index)?;
            core_artifact_doctor(&store, options)
        })
        .map_err(artifact_store_error_to_pyerr)?;
    json_serializable_to_py(py, &doctor)
}
...
enum ArtifactAddRequest {
    Node(ArtifactNodeUpsertWire),
    Link(ArtifactLinkUpsertWire),
    Payload(ArtifactPayloadWire),
}

enum ArtifactRemoveRequest {
    Node(ArtifactNodeRemoveWire),
...
fn artifact_add_request_from_pydict(
    ...
    let Some(obj) = value.as_object() else {
        return Err(PyValueError::new_err(
            "request must be a valid ArtifactNodeUpsertWire, ArtifactLinkUpsertWire, or ArtifactPayloadWire dict",
        ));
    };
    if obj.contains_key("node") {
        return serde_json::from_value(value)
            .map(ArtifactAddRequest::Node)
            .map_err(|e| {
                PyValueError::new_err(format!(
                    "request is not a valid ArtifactNodeUpsertWire dict: {e}"
                ))
            });
    }
    if obj.contains_key("link") {
        return serde_json::from_value(value)
            .map(ArtifactAddRequest::Link)
            .map_err(|e| {
                PyValueError::new_err(format!(
                    "request is not a valid ArtifactLinkUpsertWire dict: {e}"
                ))
            });
    }
    if obj.contains_key("artifact_id") && obj.contains_key("payload_type") {
        return serde_json::from_value(value)
            .map(ArtifactAddRequest::Payload)
            .map_err(|e| {
                PyValueError::new_err(format!(
                    "request is not a valid ArtifactPayloadWire dict: {e}"
                ))
            });
    }
    Err(PyValueError::new_err(
        "request must contain 'node', 'link', or ArtifactPayloadWire fields",
    ))
}

fn artifact_remove_request_from_pydict(
    request: &Bound<'_, PyDict>,
) -> PyResult<ArtifactRemoveRequest> {
    let value = py_to_json_value(request.as_any())?;
    let Some(obj) = value.as_object() else {
        return Err(PyValueError::new_err(
            "request must be a valid ArtifactNodeRemoveWire or ArtifactLinkRemoveWire dict",
        ));
    };
    if obj.contains_key("link_type")
        || obj.contains_key("target_id")
        || obj.contains_key("source_id_hint")
    {
        return serde_json::from_value(value)
            .map(ArtifactRemoveRequest::Link)
            .map_err(|e| {
                PyValueError::new_err(format!(
                    "request is not a valid ArtifactLinkRemoveWire dict: {e}"
                ))
            });
    }
    serde_json::from_value(value)
        .map(ArtifactRemoveRequest::Node)
        .map_err(|e| {
            PyValueError::new_err(format!(
                "request is not a valid ArtifactNodeRemoveWire dict: {e}"
            ))
        })
}
...
where
    T: DeserializeOwned,
{
    serde_json::from_value(py_to_json_value(value.as_any())?).map_err(|e| {
        PyValueError::new_err(format!(
            "{argument} is not a valid {wire_name} dict: {e}"
        ))
    })
}

fn json_serializable_to_py<'py, T>(
    py: Python<'py>,
    value: &T,
) -> PyResult<PyObject>
where
...
mod tests {
    ...
    #[test]
    fn artifact_bindings_round_trip_json_and_rust_queries() {
        pyo3::prepare_freethreaded_python();
        Python::with_gil(|py| {
            let index = temp_notification_path("artifact.sqlite");
            let index_str = index.to_str().unwrap();
            let node_request_obj = json_value_to_py(
                py,
                &json!({
                    "schema_version": 1,
                    "node": {
                        "id": "/tmp/artifact-binding.md",
                        "kind": "file",
                        "display_title": "artifact-binding.md",
                        "subtitle": null,
                        "provenance": "manual",
                        "source_kind": null,
                        "source_id": null,
                        "source_version": null,
                        "search_text": "artifact binding",
                        "metadata": {"lang": "markdown"},
                        "created_at": null,
                        "updated_at": null
                    },
            ...
            .unwrap();
            let node_request =
                node_request_obj.bind(py).downcast::<PyDict>().unwrap();

            let added = py_artifact_add(py, index_str, node_request).unwrap();
            let added_value = py_to_json_value(added.bind(py)).unwrap();
            assert_eq!(
                added_value,
                serde_json::to_value(ArtifactMutationResultWire {
                    schema_version: 1,
                    ...
                    affected_node_ids: vec![
                        "/tmp/artifact-binding.md".to_string()
                    ],
                    affected_link_ids: vec![],
                    tombstone_ids: vec![],
                    errors: vec![],
                })
                .unwrap()
            ...
            let link_request_obj = json_value_to_py(
                py,
                &json!({
                    "schema_version": 1,
                    "link": {
                        "id": "",
                        "link_type": "parent",
                        "source_id": "/tmp/artifact-binding.md",
                        "target_id": "/",
                        "provenance": "manual",
                        "source_kind": null,
                        "source_id_hint": null,
            ...
            .unwrap();
            let link_request =
                link_request_obj.bind(py).downcast::<PyDict>().unwrap();
            let linked = py_artifact_add(py, index_str, link_request).unwrap();
            let linked_value = py_to_json_value(linked.bind(py)).unwrap();
            assert_eq!(linked_value["links_added"], json!(1));

            let query_obj = json_value_to_py(
                py,
                &json!({
                    "schema_version": 1,
                    "text": "binding",
            ...
            .unwrap();
            let query = query_obj.bind(py).downcast::<PyDict>().unwrap();
            let listed = py_artifact_list(py, index_str, Some(query)).unwrap();
            ...
            let detail =
                py_artifact_show(py, index_str, "/tmp/artifact-binding.md")
                    .unwrap();
            ...
            .unwrap();
            let graph_options =
                graph_options_obj.bind(py).downcast::<PyDict>().unwrap();
            let graph =
            ...
            assert_eq!(doctor_value["ok"], json!(true));

            let store = core_open_artifact_store(&index).unwrap();
            let rust_detail =
                core_artifact_show(&store, "/tmp/artifact-binding.md").unwrap();
            assert_eq!(
                serde_json::to_value(rust_detail).unwrap(),
                detail_value
            );

            let remove_obj = json_value_to_py(
                py,
                &json!({
                    "schema_version": 1,
                    "id": "/tmp/artifact-binding.md",
            ...
            .unwrap();
            let remove = remove_obj.bind(py).downcast::<PyDict>().unwrap();
            let removed = py_artifact_remove(py, index_str, remove).unwrap();
            let removed_value = py_to_json_value(removed.bind(py)).unwrap();
            assert_eq!(removed_value["nodes_removed"], json!(1));
    ...
    fn artifact_bindings_cover_path_and_rebuild_request() {
        ...
        Python::with_gil(|py| {
            ...
            let artifact_file = index.with_file_name("note.md");
            fs::write(&artifact_file, "hello").unwrap();

            let upserted = py_artifact_upsert_path(
                py,
                index.to_str().unwrap(),
    ...
    #[test]
    fn artifact_binding_rejects_invalid_request_shapes() {
        pyo3::prepare_freethreaded_python();
        Python::with_gil(|py| {
            let index = temp_notification_path("artifact-invalid.sqlite");
            let bad_obj = json_value_to_py(
                py,
